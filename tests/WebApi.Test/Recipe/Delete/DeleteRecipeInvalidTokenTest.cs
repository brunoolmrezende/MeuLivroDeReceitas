...
namespace WebApi.Test.Recipe.Delete
{
    public class DeleteRecipeInvalidTokenTest : MyRecipeBookClassFixture
    {
        ...
        public async Task Error_Without_Token(string culture)
        {
            ...
            var response = await DoDelete(endpoint: $"{_endpoint}/{_recipeId}", token: token, culture: culture);
        ...
        {
            var response = await DoDelete(endpoint: $"{_endpoint}/{_recipeId}", token: "InvalidToken", culture: culture);
