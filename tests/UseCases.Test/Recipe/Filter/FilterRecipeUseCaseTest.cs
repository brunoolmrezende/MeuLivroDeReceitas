...
namespace UseCases.Test.Recipe.Filter
{
    public class FilterRecipeUseCaseTest
    {
        ...
        public async Task Error_Invalid_CookingTime()
        {
            ...
            await act.Should().ThrowAsync<ErrorOnValidationException>()
                .Where(error => error.GetErrorMessages().Count == 1 &&
                    error.GetErrorMessages().Contains(ResourceMessagesException.COOKING_TIME_NOT_SUPPORTED));
        }
            ...
            IList<MyRecipeBook.Domain.Entities.Recipe> recipes)
        {
            ...
            return new FilterRecipeUseCase(loggedUser, recipeReadOnlyRepository, mapper, blobStorage);
        }
    }
}
